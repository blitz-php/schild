...
class AccessTokens extends BaseAuthenticator implements AuthenticatorInterface
{
    ...
    public function check(array $credentials): Result
    {
        if (! array_key_exists('token', $credentials) || empty($credentials['token'])) {
            return new Result([
                'success' => false,
                'reason'  => lang('Auth.noToken', [config('auth.authenticator_header.tokens')]),
            ]);
        }
    ...
    public function loggedIn(): bool
    {
        ...
        return $this->attempt([
            'token' => $request->getHeaderLine(config('auth.authenticator_header.tokens')),
        ])->isOK();
    }
    ...
    public function getBearerToken(): ?string
    {
        ...
        $header = $request->getHeaderLine(config('auth.authenticator_header.tokens'));
